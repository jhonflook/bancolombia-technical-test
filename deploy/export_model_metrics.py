...
def _extract_params(run) -> dict:
    ...
    best_params: dict = {}
    skip = {"model_type", "n_features", "n_trials", "scale_pos_weight"}
    for k, v in params.items():
        if k not in skip:
            try:
                ...
                best_params[k] = v
    return {
        "model_name":       params.get("model_type", run.data.tags.get("model_type", "")),
...
def _extract_split_strategy(run, client: mlflow.MlflowClient) -> str:
    """Busca split_strategy en el run padre."""
...
def main() -> None:
    ...
    for run in child_runs:
        ...
        feature_rows.extend(_fetch_feature_importance(run, client, args.top_features))
        logger.info("Procesado run %s | %s | split=%s | test_auc=%.4f",
                    rid[:8], params["model_name"], split,
